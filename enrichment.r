...
# Load specific functions from the tidytree package
...
perform_go_analysis <- function(fcvals_up, annot, universe, lfc_threshold = 0, condition) {
    ...
    min_value <- min(degenes$logFC, na.rm = TRUE)
    max_value <- max(degenes$logFC, na.rm = TRUE)
    # Correct way to print them together
    print(paste("Minimum FC:", 2^min_value, "Maximum FC:", 2^max_value))
    print(paste("Minimum logFC:", min_value, "Maximum logFC:", max_value))
...
generate_chord_diagram <- function(go_results, genelist, degenes, condition) {
    ...
    df <- as.data.frame(chord)
    min_value <- min(df$logFC, na.rm = TRUE)
    max_value <- max(df$logFC, na.rm = TRUE)
    # Correct way to print them together
    print(paste("==>Minimum FC:", 2^min_value, "Maximum FC:", 2^max_value))
    print(paste("==>Minimum logFC:", min_value, "Maximum logFC:", max_value))
